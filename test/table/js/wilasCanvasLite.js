...
(function(_global){
	...
	var loop = function(_self){
		_self.heartBeat = window.requestAnimationFrame(function(){
			loop(_self);
	...
	Layer.prototype = {
		...
		clear:function(rect,notShowBg){
			this.changed = false;
			if (this.option && (this.option.background || this.option.draw)){
				rect = null;
			}
			...
			if (!isZeroRect(rect)){
				...
				if (this.option && this.option.background && !notShowBg){
					this.ctx.save();
				...
				if (this.option && this.option.draw){
					this.option.draw.call(null,this);
				}
			}
