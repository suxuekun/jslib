...
//	console.log("visible:",this.heatmapVisiable)
...
//				console.log(key,this.chartName);
...
//		console.log('a',param,this);
	...
	var start_drag = function(event){
		...
		if (event.type == "touchstart"){
			this.pre_point={x:event.touches[0].clientX,y:event.touches[0].clientY};
		}
		this.draging = true;
	} 
	var move_drag = function(event){
		if (this.draging){
			var x_change = 0;
			var y_change = 0;
			if (event.type == "touchmove"){
				x_change = event.touches[0].clientX - this.pre_point.x;
				y_change = event.touches[0].clientY - this.pre_point.y;
				this.pre_point = {x:event.touches[0].clientX,y:event.touches[0].clientY};
			}else{
				x_change = event.offsetX-this.pre_mouse_event.offsetX;
				y_change = event.offsetY-this.pre_mouse_event.offsetY;
				this.pre_mouse_event = event;
			}
	...
	this.panzoomRoot.find(this.chartName).bind("mousedown",$.proxy(start_drag,this))
	this.panzoomRoot.find(this.chartName).bind("pointerdown",$.proxy(start_drag,this))
	this.panzoomRoot.find(this.chartName).bind("touchstart",$.proxy(start_drag,this))
	
	this.panzoomRoot.find(this.chartName).bind("mousemove",$.proxy(move_drag,this))
	this.panzoomRoot.find(this.chartName).bind("pointermove",$.proxy(move_drag,this))
	this.panzoomRoot.find(this.chartName).bind("touchmove",$.proxy(move_drag,this))
	
	this.panzoomRoot.find(this.chartName).bind("mouseup",$.proxy(end_drag,this))
	this.panzoomRoot.find(this.chartName).bind("pointerup",$.proxy(end_drag,this))
	this.panzoomRoot.find(this.chartName).bind("touchend",$.proxy(end_drag,this))
